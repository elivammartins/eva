  ...
  const startVoiceSession = async () => {
    ...
    try {
      ...
      if (!apiKey || apiKey === 'undefined' || apiKey === '') {
        ...
        setApiErrorMessage("A variável API_KEY está vazia ou não foi configurada no Vercel/AI Studio.");
      ...
      const sessionPromise = ai.live.connect({
        ...
        callbacks: {
          ...
          onerror: (e: any) => { 
            ...
            const errMsg = e.message || "Erro de handshake com a API Gemini Live";
            setApiErrorMessage(errMsg);
            
            if (errMsg.includes("entity was not found") || errMsg.includes("404")) {
              setStatusLog('CHAVE INVÁLIDA');
            } else if (errMsg.includes("403") || errMsg.includes("permission") || errMsg.includes("billing")) {
              setStatusLog('BILLING REQUERIDO');
            } else {
              setStatusLog('ERRO DE LINK');
            }
      ...
      setApiErrorMessage(err.message || "Erro ao acessar microfone ou hardware de áudio.");
      setStatusLog('ERRO DE HARDWARE');
      cleanupAudioResources();
    }
  };
    ...
    <div className="h-full w-full bg-black text-white flex flex-col overflow-hidden relative font-sans italic">
      ...
      <header className="h-[65px] bg-black/90 border-b border-white/5 flex items-center px-5 gap-5 z-[500] pointer-events-auto shrink-0 relative">
        <div className="flex items-center gap-3 pr-5 border-r border-white/10 shrink-0">
           ...
           <span className="text-[10px] font-black uppercase tracking-widest text-white/40">CORE V63 DEBUG</span>
        </div>
      ...
      <AddStopModal isOpen={isAddStopModalOpen} onClose={() => setIsAddStopModalOpen(false)} onAdd={(n, la, ln) => {
          ...
          setActiveApp('nav');
          window.location.href = `google.navigation:q=${la},${ln}`;
      }} />
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center">
              <span className="text-[10px] font-black uppercase tracking-widest text-white flex items-center gap-2">
                <i className="fas fa-exclamation-triangle"></i> FALHA DE AUTENTICAÇÃO
              </span>
              <button onClick={() => setApiErrorMessage(null)} className="text-white/40 hover:text-white"><i className="fas fa-times"></i></button>
            </div>
            <p className="text-[10px] font-bold text-white/90 leading-relaxed bg-black/20 p-3 rounded-xl border border-white/5 uppercase italic">
              DEBUG: {apiErrorMessage}
            </p>
            <div className="flex gap-3">
              <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="flex-1 h-12 bg-black/40 rounded-2xl flex items-center justify-center text-[9px] font-black uppercase border border-white/10 hover:bg-black/60 transition-all">Ver Faturamento</a>
              <button onClick={() => {
                setApiErrorMessage(null);
            ...
            <p className="text-[7px] text-center text-white/40 font-black uppercase tracking-widest">Dica: Selecione o projeto com nível pago 1 ativo.</p>
          </div>
        </div>
      )}
    </div>
